...
public class CustomEventHandler {
    ...
    public void handleDeathMatchEvent() {
        if (!config.get(MainConfig.ENABLE_DEATHMATCH)) {
            return;
        }

        List<String> deathMatchCommands = config.get(MainConfig.DEATHMATCH_COMMANDS);


        deathMatchCommands.forEach(cmd -> {
            if (cmd.startsWith("/")) {
                cmd = cmd.substring(1);
            }
            ...
            try {
                ...
                Bukkit.getLogger().warning("[UhcCore] Failed to execute death match command: " + cmd);
                exception.printStackTrace();
            }
        });
    }

    public void handleStartingGameEvent() {

        List<String> startingGameCommands = config.get(MainConfig.STARTING_GAME_COMMANDS);


        startingGameCommands.forEach(cmd -> {
            if (cmd.startsWith("/")) {
                cmd = cmd.substring(1);
            }
            ...
            try {
                ...
                Bukkit.getLogger().warning("[UhcCore] Failed to execute starting game command: " + cmd);
                exception.printStackTrace();
            }
        });
    }
