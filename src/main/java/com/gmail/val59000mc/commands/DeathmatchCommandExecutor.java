...
public class DeathmatchCommandExecutor implements CommandExecutor{
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length != 1 || (!(args[0].equalsIgnoreCase("start")) && !(args[0].equalsIgnoreCase("instant")))){
            sender.sendMessage(ChatColor.RED + "Usage: /deathmatch start or /deathmatch instant");
            return true;
        }
        ...
        if (gameManager.getRemainingTime() == 0 && !gameManager.getConfig().get(MainConfig.ENABLE_DEATHMATCH)){
            if (args[0].equalsIgnoreCase("start")) {
                ...
                gameManager.setRemainingTime(11);
            }
