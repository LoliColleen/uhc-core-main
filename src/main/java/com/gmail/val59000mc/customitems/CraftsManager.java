...
public class CraftsManager {
	...
	public static Map<Long, Craft> getCraftMap(){
		return craftMap;
	}
	public static Map<Long, Craft> getCraftNeed(){
		return craftNeed;
	}
	...
	public static void loadCrafts(){
		...
		for(String name : craftsKeys){
			...
			ItemStack craftItem;
			int limit;
			boolean defaultName, reviveItem, reviveWithInventory, shapeless, unbreakable;
			...
			try{
				...
				shapeless = section.getBoolean("shapeless", false);
				unbreakable = section.getBoolean("unbreakable", false);
				if (reviveItem) {
					defaultName = false;
				}

				Craft craft = new Craft(name, recipe, craftItem, limit, defaultName, shapeless, unbreakable);
				...
				//craftMap.put(craft.hashCode(),craft);
				if(craft.isShapeless()){
					craftMap.put(craft.shapelessHashCode(recipe),craft);
				}else{
					craftMap.put(craft.itemStacksHashCode(recipe),craft);
				}

				crafts.add(craft);

				//get the materials which the craft needs

				craftNeed.put(craft.recipeNeedHashCode(recipe),craft);
	...
	public static void saveCraft(Craft craft){
		...
		// shapeless
		cfg.set(
				"custom-crafts." + craft.getName() + ".shapeless",
				craft.isShapeless()
		);

		// unbreakable
		cfg.set(
				"custom-crafts." + craft.getName() + ".unbreakable",
				craft.isUnbreakable()
		);
