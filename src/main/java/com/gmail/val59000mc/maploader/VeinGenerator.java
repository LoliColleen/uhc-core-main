...
public class VeinGenerator {
	...
	private Block tryAdjustingToProperBlock(Block randBlock) {
		if(randBlock.getType().equals(Material.STONE)||randBlock.getType().equals(Material.DEEPSLATE)){
			return randBlock;
		}
		...
		for(int i = -5; i<=5 ; i++){
			for(int j = -5; j<=5 ; j++){
				for(int k = -5; k<=5 ; k++){
					...
					if(relativeBlock.getType().equals(Material.STONE)||relativeBlock.getType().equals(Material.DEEPSLATE)){
