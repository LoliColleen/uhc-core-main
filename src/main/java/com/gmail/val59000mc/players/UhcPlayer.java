...
public class UhcPlayer {
	...
	private final Map<UhcPlayer,Long> compassPlayingLastUpdate = new HashMap<>();
	private final Map<UhcPlayer,Long> cornUsingLastUpdate = new HashMap<>();
	...
	public void ArtemisArrowLock(Arrow arrow,List<UhcPlayer> pointPlayers){

		double nearestDistance = 10;
		Player realPlayerAiming = null;


		for (UhcPlayer pointPlayer : pointPlayers) {
			double distance;
			try {
				distance = arrow.getLocation().distance(pointPlayer.getPlayer().getLocation());
				if (nearestDistance > distance
						&& arrow.getWorld().getEnvironment() == pointPlayer.getPlayer().getWorld().getEnvironment()
						&& distance <= 5) {
					realPlayerAiming = pointPlayer.getPlayer();
		...
		if (realPlayerAiming != null){
			Location locA = arrow.getLocation();
			Location locB = realPlayerAiming.getLocation().add(0,1,0);
			org.bukkit.util.Vector vectorAB = locB.clone().subtract(locA).toVector();
			vectorAB.normalize();
			vectorAB = vectorAB.clone().multiply(arrow.getVelocity().length());
			arrow.setVelocity(vectorAB);
			arrow.addScoreboardTag("Artemis Arrow Locked");
		}
	}

	public void pointMasterCompassToPlayer(int cooldown,UhcPlayer user){
		PlayerManager pm = GameManager.getGameManager().getPlayerManager();

		// Check cooldown
		if (cooldown != -1 && (cooldown*TimeUtils.SECOND) + compassPlayingLastUpdate.getOrDefault(user,-1L) > System.currentTimeMillis()){
		...
		if((pointPlayers.size() == 1 && pointPlayers.get(0).equals(this)) || pointPlayers.size() == 0){
			...
			compassPlayingLastUpdate.put(user,System.currentTimeMillis());

			try {
				Player bukkitPlayer = getPlayer();
				double nearestDistance = 2147483647;
				Player realPlayerPointing = null;

				for (UhcPlayer pointPlayer : pointPlayers) {
					double distance = (int) bukkitPlayer.getLocation().distance(pointPlayer.getPlayer().getLocation());
					if (nearestDistance > distance
							&& bukkitPlayer.getWorld().getEnvironment() == pointPlayer.getPlayer().getWorld().getEnvironment()
							&& ((pointPlayer.getPlayer().getWorld().getEnvironment() != World.Environment.NORMAL
							|| pointPlayer.getPlayer().getLocation().getY() >= 63))) {
						realPlayerPointing = pointPlayer.getPlayer();
						nearestDistance = distance;
					}
				}

				if(realPlayerPointing != null){
					buildMasterCompassLine(bukkitPlayer,bukkitPlayer.getLocation(),realPlayerPointing.getLocation());
				}else{
					sendMessage(Lang.ITEMS_COMPASS_PLAYING_ERROR);
				}

			} catch (UhcPlayerNotOnlineException e) {
				sendMessage(Lang.TEAM_MESSAGE_PLAYER_NOT_ONLINE);
			}
		}
	}

	public static void buildMasterCompassLine(Player bukkitPlayer, Location locA, Location locB) {
		new BukkitRunnable(){
			int i = 20;
			public void run(){
				if (i <= 0){
				...
				else
				{
					...
					for (double i = 0; i < 20; i += 1) {
						Vector vector = vectorAB.clone().multiply(i/2);
						locA.add(vector);
						locA.add(0,1,0);
						Objects.requireNonNull(locA.getWorld()).spawnParticle(Particle.FLAME,locA,16,0.5,0.5,0.5,0);
						locA.add(0,-1,0);
		...
		}.runTaskTimerAsynchronously(UhcCore.getPlugin(UhcCore.class),0,10);
		ItemStack hand = bukkitPlayer.getInventory().getItemInMainHand();
		bukkitPlayer.getWorld().playSound(bukkitPlayer.getLocation(),Sound.ENTITY_ITEM_BREAK,1,1);
		if (hand.getAmount() > 0) {
			hand.setAmount(hand.getAmount() - 1);
		}
	}

	public static AreaEffectCloud placeAEC(int duration, Particle particle,Float radius,Location location){
		var aec = Objects.requireNonNull(location.getWorld()).spawn(location, AreaEffectCloud.class);
	...
	public void regenPlayerCorn(int cooldown,UhcPlayer user){
		// Check cooldown
		if (cooldown != -1 && (cooldown*TimeUtils.SECOND) + cornUsingLastUpdate.getOrDefault(user,-1L) > System.currentTimeMillis()){
			return;
		}

		try {
			cornUsingLastUpdate.put(user,System.currentTimeMillis());
			Player p = getPlayer();
	...
	public void pointCompassToNextPlayer(int mode, int cooldown, UhcPlayer user) {
		PlayerManager pm = GameManager.getGameManager().getPlayerManager();
