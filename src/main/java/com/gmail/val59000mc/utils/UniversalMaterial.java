...
public enum UniversalMaterial{
    ...
    public static boolean isBed(Material tool) {
        return tool == Material.WHITE_BED ||
                tool == Material.ORANGE_BED ||
                tool == Material.MAGENTA_BED ||
                tool == Material.LIGHT_BLUE_BED ||
                tool == Material.YELLOW_BED ||
                tool == Material.LIME_BED ||
                tool == Material.PINK_BED ||
                tool == Material.GRAY_BED ||
                tool == Material.LIGHT_GRAY_BED ||
                tool == Material.CYAN_BED ||
                tool == Material.PURPLE_BED ||
                tool == Material.BLUE_BED ||
                tool == Material.BROWN_BED ||
                tool == Material.GREEN_BED ||
                tool == Material.RED_BED ||
                tool == Material.BLACK_BED ;
    }

    public static boolean isDoor(Material tool) {
        return tool == Material.IRON_DOOR ||
                tool == Material.OAK_DOOR ||
                tool == Material.SPRUCE_DOOR ||
                tool == Material.BIRCH_DOOR ||
                tool == Material.JUNGLE_DOOR ||
                tool == Material.ACACIA_DOOR ||
                tool == Material.DARK_OAK_DOOR ||
                tool == Material.CRIMSON_DOOR ||
                tool == Material.WARPED_DOOR ;
    }

    public static boolean isMeat(Material tool) {
        return tool == Material.PORKCHOP ||
                tool == Material.COD ||
                tool == Material.SALMON ||
                tool == Material.BEEF ||
                tool == Material.CHICKEN ||
                tool == Material.RABBIT ||
                tool == Material.POTATO ||
                tool == Material.MUTTON ;
    }

    public static boolean isAnvil(Material tool) {
        return tool == Material.ANVIL ||
                tool == Material.CHIPPED_ANVIL ||
                tool == Material.DAMAGED_ANVIL ;
    }

    public static boolean isDisc(Material tool) {
        return tool == Material.MUSIC_DISC_11 ||
                tool == Material.MUSIC_DISC_13 ||
                tool == Material.MUSIC_DISC_BLOCKS ||
                tool == Material.MUSIC_DISC_CAT ||
                tool == Material.MUSIC_DISC_CHIRP ||
                tool == Material.MUSIC_DISC_FAR ||
                tool == Material.MUSIC_DISC_MALL ||
                tool == Material.MUSIC_DISC_MELLOHI ||
                tool == Material.MUSIC_DISC_PIGSTEP ||
                tool == Material.MUSIC_DISC_STAL ||
                tool == Material.MUSIC_DISC_STRAD ||
                tool == Material.MUSIC_DISC_WAIT ||
                tool == Material.MUSIC_DISC_WARD ;
    }

    public static boolean isSapling(Material tool) {
        return tool == Material.OAK_SAPLING ||
                tool == Material.SPRUCE_SAPLING ||
                tool == Material.ACACIA_SAPLING ||
                tool == Material.BAMBOO_SAPLING ||
                tool == Material.BIRCH_SAPLING ||
                tool == Material.DARK_OAK_SAPLING ||
                tool == Material.JUNGLE_SAPLING ;
    }
